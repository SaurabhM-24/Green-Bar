	...
	>
		...
		<div class="flex justify-between items-start gap-4">
			<div class="flex flex-col gap-1 w-full">
				...
				<textarea rows="2" style="field-sizing: content; min-width: 2ch;" bind:value={addData.category} class="bg-transparent font-display text-3xl text-white tracking-wide pr-14 leading-tight focus:outline-none placeholder-gray-600 border-b border-transparent hover:border-gray-700 focus:border-white transition-colors pb-1 resize-none" placeholder="Category Name"></textarea>
			</div>
			<button class="absolute top-6 right-6 p-2 text-gray-400 hover:text-white transition-colors bg-[#222] rounded-xl box-3d shrink-0 z-50" onclick={onclose} disabled={loading}>
				<X class="w-5 h-5" />
			</button>
		</div>
		...
		<div class="flex flex-col gap-5 mt-2">
			...
			<div class="flex flex-col gap-1.5 mt-1">
				...
				<textarea bind:value={addData.description} class="bg-transparent text-gray-400 text-base leading-relaxed focus:outline-none w-full resize-none min-h-[48px] p-0 border-b border-transparent hover:border-gray-700 focus:border-white transition-colors" placeholder="Add a description..."></textarea>
			</div>
						...
						<div class="flex items-center gap-3 overflow-hidden">
							{#if addData.icon_name && iconMap[addData.icon_name]}
								<picture>
									...
									{/if}
									<img src={iconMap[addData.icon_name].webp} alt="" class="h-5 w-5 object-contain shrink-0" />
								</picture>
							{/if}
							<span class="text-gray-100 font-medium truncate text-base">{addData.icon_name || 'Select Icon'}</span>
						</div>
						...
						<div class="absolute right-0 top-full mt-2 w-56 max-h-64 overflow-y-auto bg-[#1a1a1a] rounded-xl box-3d z-50 p-2 flex flex-col gap-1" transition:slide={{ duration: 250, easing: cubicOut }}>
							{#each iconsList as icon}
								<button 
									...
									onclick={() => { addData.icon_name = icon.name; isIconDropdownOpen = false; }}
								>
									<picture>
										{#if icon.avif}
