		...
		>
			...
			<div class="flex flex-col gap-5 mt-2">
				...
				<div class="flex flex-col gap-1.5 mt-1">
					...
					{:else}
						<span
							class="text-gray-400 text-base leading-relaxed {transaction.description
								? ''
				...
				<div class="flex gap-4">
					<div class="flex-1 flex flex-col gap-1.5 border-r border-gray-800/60 pr-4">
						...
						{#if isEditing}
							<input
								type="date"
								bind:value={editData.transaction_date}
						...
						{:else}
							<span
								class="text-gray-200 text-base border-b border-transparent w-full inline-block pb-1"
					...
					<div class="flex-1 flex flex-col gap-1.5 relative pl-2">
							...
							{#if isTypeDropdownOpen}
								...
								>
									{#each ['Debit', 'Credit'] as tOpt}
										<button
											class="text-left px-4 py-2 text-base tracking-wide rounded-lg transition-colors {editData.transaction_type?.toLowerCase() ===
											tOpt.toLowerCase()
												? 'bg-white text-black box-3d'
												: 'text-gray-200 hover:bg-[#2a2a2a]'} capitalize"
											onclick={() => {
												editData.transaction_type = tOpt;
						...
						{:else}
							<span
								class="text-gray-200 text-base border-b border-transparent w-full inline-block capitalize pb-1"
				...
				<div class="flex gap-4 items-end mt-1">
					...
					<div class="flex-1 flex flex-col gap-1.5 min-w-0 border-r border-gray-800/60 pr-4">
						...
						{#if isEditing}
							<div
								class="flex flex-row flex-nowrap items-center text-4xl tracking-wide font-bold {isEditDebit
									? 'text-[#ff6b6b]'
							...
							>
								...
								<input
									...
									class="bg-transparent focus:outline-none flex-1 min-w-0 p-0 m-0 [-moz-appearance:_textfield] [&::-webkit-outer-spin-button]:m-0 [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:m-0 [&::-webkit-inner-spin-button]:appearance-none"
								/>
							</div>
						{:else}
							<span
								class="text-4xl tracking-wide font-bold {isDebit
									? 'text-[#ff6b6b]'
									: 'text-[#69db7c]'} border-b border-transparent w-full inline-block truncate pb-0.5"
							>
									...
									{#if categories.find((c) => c.category === editData.category)?.icon_name && iconMap[categories.find((c) => c.category === editData.category)?.icon_name]}
										<picture>
											{#if iconMap[categories.find((c) => c.category === editData.category)?.icon_name].avif}
												<source
													srcset={iconMap[
														categories.find((c) => c.category === editData.category)?.icon_name
													].avif}
													type="image/avif"
												/>
											{/if}
							...
							{#if isCategoryDropdownOpen}
								...
								>
									{#each categories.filter((c) => c.category !== 'All') as cat}
										<button
											class="flex items-center gap-3 text-left px-4 py-3 text-base tracking-wide rounded-lg transition-colors {editData.category ===
											cat.category
												? 'bg-white text-black box-3d'
												: 'text-gray-200 hover:bg-[#2a2a2a]'}"
											onclick={() => {
												editData.category = cat.category;
								...
								{#if categories.find((c) => c.category === transaction.category)?.icon_name && iconMap[categories.find((c) => c.category === transaction.category)?.icon_name]}
									<picture>
										{#if iconMap[categories.find((c) => c.category === transaction.category)?.icon_name].avif}
											<source
												srcset={iconMap[
													categories.find((c) => c.category === transaction.category)?.icon_name
												].avif}
												type="image/avif"
											/>
										{/if}
