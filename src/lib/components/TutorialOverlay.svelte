<script>
	...
	let modalTop = $derived.by(() => {
		...
		if (spaceBelow >= modalHeight + 20) return `${targetRect.bottom + 20}px`;
		if (spaceAbove >= modalHeight + 20) return `${targetRect.top - modalHeight - 20}px`;
		return '50%';
	});

	let modalTransform = $derived.by(() => {
		if (!targetRect) return 'translateY(-50%)';
		const spaceBelow = window.innerHeight - targetRect.bottom;
		const spaceAbove = targetRect.top;
	...
	const steps = [
		{
			...
			desc: 'Manage your global app settings and profile here.'
		},
		{
			page: '/',
			selector: 'a[href="/variable"]',
			title: 'Variable Expenses',
			desc: 'Track your day-to-day spending limits here.'
		},
		{
			page: '/',
			selector: 'a[href="/fixed"]:not(.mt-2)',
			title: 'Fixed Expenses',
			desc: 'Manage your recurring full-payments.'
		},
		{
			page: '/',
			selector: 'a[href="/fixed"].mt-2',
			title: 'Account Status',
			desc: 'See your unallocated Personal Corpus and total balance.'
		},
		{
			page: '/',
			selector: 'nav',
			title: 'Navigation',
			desc: 'Use the bottom bar to switch views.'
		},
		{
			page: '/',
			selector: 'a[href="/add"]',
			title: 'Add Transaction',
			desc: 'Floating button to quickly add a transaction.'
		},

		{
			...
			desc: 'Edit order or add categories.'
		},
		{
			page: '/variable',
			selector: '#variable-list > div:first-child',
			title: 'Modify Category',
			desc: 'Tap on any category to view details or modify it.'
		},

		{
			...
			title: 'Fixed Options',
			desc: 'Manage your fixed responsibilities.'
		},
		{
			page: '/fixed',
			selector: '#corpus-list > div:first-child',
			title: 'Corpus Funds',
			desc: 'Tap on a corpus fund to modify or view details.'
		},
		{
			page: '/fixed',
			selector: '#fixed-list > div:first-child',
			title: 'Fixed Item',
			desc: 'Tap on any fixed expense to manage its payment status.'
		},

		{
			page: '/list',
			selector: 'button:has(.lucide-sliders-horizontal), .relative.z-40 > button',
			title: 'Filters',
			desc: 'Filter history by date and category.'
		},
		{
			page: '/list',
			selector: '#transaction-list > div:first-child',
			title: 'Modify Transaction',
			desc: 'Tap any transaction to view details or modify it.'
		},

		{
			page: '/add',
			selector: '#add-card-container',
			title: 'Transaction Details',
			desc: 'Fill out the details of your transaction here before proceeding.'
		},
		{
			page: '/add',
			selector: 'button.w-full.py-4',
			title: 'Save',
			desc: 'Click here to save the transaction to your history.'
		},
		{
			page: '/',
			selector: null,
			title: 'You are all set!',
			desc: 'You have completed the tutorial. Would you like to restart or finish?'
		}
	];
	...
	async function restartTutorial() {
		...
		if ($page.url.pathname !== steps[0].page) {
			await goto(steps[0].page);
...
{#if !isPageTransitioning}
	<div class="fixed inset-0 z-[200] pointer-events-auto" transition:fade={{ duration: 200 }}>
		...
		{#if targetRect || step === steps.length}
			<div
					...
					<div class="flex gap-4 items-center">
						{#if step === steps.length}
							<button
								...
								onclick={restartTutorial}>Restart</button
							>
							<button
