...
<button
	class="w-full text-left relative py-6 px-8 flex justify-between items-center after:absolute after:bottom-0 after:left-8 after:right-8 after:h-px after:bg-gray-800/60 last:after:hidden transition-all active:scale-[0.98] focus:outline-none {isOutOfCycle
		? 'bg-[#0f0f0f]'
		: 'bg-[#0f0f0f] hover:bg-[#151515]'}"
	style={isOutOfCycle
		? 'background: repeating-linear-gradient(-45deg, transparent, transparent 10px, #151515 10px, #151515 20px) #0f0f0f;'
		: ''}
	{onclick}
>
	<div class="flex flex-col overflow-hidden mr-6">
		<span class="text-gray-200 text-xl tracking-wide truncate {description ? 'mb-1.5' : ''}"
			>{title}</span
		>
		{#if description}
