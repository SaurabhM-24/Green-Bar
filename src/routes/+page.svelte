...
>
	...
	{:else}
		...
		>
			...
			{#if budgets.length === 0}
				<div class="flex gap-4">
					<button class="flex-1 py-3 bg-[#1a1a1a] rounded-xl flex items-center justify-center gap-2 hover:bg-[#2a2a2a] transition-colors" onclick={(e) => { e.preventDefault(); isAddVariableModalOpen = true; }}>
						<Plus class="w-5 h-5 text-gray-400" />
						<span class="text-sm text-gray-300">Add</span>
					</button>
					<button class="flex-1 py-3 bg-[#1a1a1a] rounded-xl flex items-center justify-center gap-2 hover:bg-[#2a2a2a] transition-colors" onclick={(e) => { e.preventDefault(); showHideVariableModal = true; }}>
			...
			{:else if budgets.length < 3}
				<div class="flex flex-col gap-6">
					{#each budgets as b, index}
						<div class="flex items-center gap-4" in:fly={{ y: 20, duration: 400, delay: index * 100 + 100 }}>
							<div class="shrink-0 w-12 h-12 bg-[#1a1a1a] rounded-2xl flex items-center justify-center">
								{#if b.icon_name && iconMap[b.icon_name]}
									<picture class="flex items-center justify-center w-full h-full p-2.5">
										...
										{/if}
										<img src={iconMap[b.icon_name].webp} alt="{b.category} icon" class="w-full h-full object-contain" />
									</picture>
								{:else}
		...
		>
			...
			{#if fixedBudgets.length === 0}
				<div class="flex gap-4">
					<button class="flex-1 py-3 bg-[#1a1a1a] rounded-xl flex items-center justify-center gap-2 hover:bg-[#2a2a2a] transition-colors" onclick={(e) => { e.preventDefault(); isAddFixedModalOpen = true; }}>
						<Plus class="w-5 h-5 text-gray-400" />
						<span class="text-sm text-gray-300">Add</span>
					</button>
					<button class="flex-1 py-3 bg-[#1a1a1a] rounded-xl flex items-center justify-center gap-2 hover:bg-[#2a2a2a] transition-colors" onclick={(e) => { e.preventDefault(); showHideFixedModal = true; }}>
						<EyeOff class="w-5 h-5 text-gray-400" />
						<span class="text-sm text-gray-300">Hide Card</span>
					</button>
				</div>
			{:else if fixedBudgets.length < 3}
				<div class="flex flex-col gap-4">
					{#each fixedBudgets as b, index}
						<div class="flex items-center justify-between p-3 bg-[#1a1a1a] rounded-2xl" in:fly={{ y: 20, duration: 400, delay: index * 100 + 100 }}>
									...
									{#if b.icon_name && iconMap[b.icon_name]}
										<picture class="w-full h-full flex items-center justify-center">
											{#if iconMap[b.icon_name].avif}
												<source srcset={iconMap[b.icon_name].avif} type="image/avif" />
											{/if}
											<img src={iconMap[b.icon_name].webp} alt="{b.category} icon" class="w-[80%] h-[80%] object-contain" />
										</picture>
									{/if}
								</div>
	...
	{#if isAddVariableModalOpen}
		<AddCategoryModal onclose={() => (isAddVariableModalOpen = false)} onsave={handleAddVariableSave} />
	{/if}
	...
	{#if showHideVariableModal}
		<div transition:fade={{ duration: 200 }} class="fixed inset-0 bg-black/40 backdrop-blur-sm z-[100] overflow-y-auto">
			<div class="min-h-full flex items-center justify-center p-4">
				<div transition:scale={{ start: 0.95, duration: 250, easing: cubicOut }} class="bg-[#151515] w-full max-w-md rounded-3xl p-6 md:p-8 box-3d flex flex-col gap-6 relative">
					<h2 class="text-2xl font-display text-white tracking-wide">Hide Variable Expenses</h2>
					<p class="text-gray-400 text-base leading-relaxed">
						This card will be hidden and can be brought back by adding a category from the respective page.
					</p>
					<div class="flex gap-4 mt-2">
						<button class="flex-1 py-3.5 rounded-xl bg-[#222] hover:bg-[#2a2a2a] text-white font-medium box-3d tracking-wide transition-all active:translate-y-1" onclick={() => (showHideVariableModal = false)}>Cancel</button>
						<button class="flex-1 py-3.5 rounded-xl bg-[#ff6b6b] hover:bg-[#ff8787] text-black font-bold box-3d tracking-wide transition-all active:translate-y-1" onclick={confirmHideVariable}>Hide</button>
					</div>
				</div>
			</div>
		</div>
		...
		<div transition:fade={{ duration: 200 }} class="fixed inset-0 bg-black/40 backdrop-blur-sm z-[100] overflow-y-auto">
			<div class="min-h-full flex items-center justify-center p-4">
				<div transition:scale={{ start: 0.95, duration: 250, easing: cubicOut }} class="bg-[#151515] w-full max-w-md rounded-3xl p-6 md:p-8 box-3d flex flex-col gap-6 relative">
					...
					<div class="flex gap-4 mt-2">
						<button class="flex-1 py-3.5 rounded-xl bg-[#222] hover:bg-[#2a2a2a] text-white font-medium box-3d tracking-wide transition-all active:translate-y-1" onclick={() => (showHideFixedModal = false)}>Cancel</button>
						<button class="flex-1 py-3.5 rounded-xl bg-[#ff6b6b] hover:bg-[#ff8787] text-black font-bold box-3d tracking-wide transition-all active:translate-y-1" onclick={confirmHideFixed}>Hide</button>
					</div>
				</div>
			</div>
		</div>
