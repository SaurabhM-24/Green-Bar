<script>
	...
	$effect(() => {
		if (!appData.loading) {
			categories = [...appData.budgets, ...appData.corpusBudgets, ...appData.fixedBudgets];
		}
	});
...
<div
	...
	class="col-start-1 row-start-1 min-w-0 w-full px-4 pt-16 relative min-h-full flex flex-col items-center"
>
	{#if showError}
		<div
		...
		>
			<div
				class="bg-[#1a1a1a] border-l-4 border-[#ff6b6b] text-[#ff6b6b] px-6 py-4 rounded-xl box-3d shadow-xl tracking-wide text-sm flex-1 text-center font-medium"
			>
				{errorMessage}
			</div>
		</div>
	...
	>
		...
		<div class="flex flex-col gap-1">
			<h1 class="text-xs font-semibold text-gray-500 uppercase tracking-wider pl-1">
				New Transaction
			</h1>
			<input
				type="text"
				bind:value={title}
				class="bg-transparent font-display text-3xl text-white tracking-wide leading-tight w-full focus:outline-none placeholder-gray-600 border-b border-transparent hover:border-gray-700 focus:border-white transition-colors pb-1"
				placeholder="Title"
			/>
		</div>
		...
		<div class="flex flex-col gap-5 mt-2">
			...
			<div class="flex gap-4">
				<div class="flex-1 flex flex-col gap-1.5 border-r border-gray-800/60 pr-4">
					...
					<input
						type="date"
						bind:value={date}
						class="bg-transparent text-gray-200 text-base focus:outline-none border-b border-transparent hover:border-gray-700 focus:border-white transition-colors cursor-pointer w-full pb-1"
					/>
				</div>
					...
					{#if isTypeDropdownOpen}
						...
						>
							{#each ['debit', 'credit'] as tOpt}
								<button
									class="text-left px-4 py-2 text-base tracking-wide rounded-lg transition-colors {type ===
									tOpt
										? 'bg-white text-black box-3d'
										: 'text-gray-200 hover:bg-[#2a2a2a]'} capitalize"
									onclick={() => {
										type = tOpt;
			...
			<div class="flex gap-4 items-end mt-1">
				...
				<div class="flex-1 flex flex-col gap-1.5 min-w-0 border-r border-gray-800/60 pr-4">
					...
					>
					<div
						class="flex flex-row flex-nowrap items-baseline text-4xl tracking-wide font-bold {isDebit
							? 'text-[#ff6b6b]'
							: 'text-[#69db7c]'} border-b border-transparent hover:border-gray-700 focus-within:border-current transition-colors w-full pb-0.5"
					>
						<span class="mr-1 shrink-0">{isDebit ? '-' : '+'}₹</span>
						<input
							type="number"
							bind:value={amount}
							class="bg-transparent focus:outline-none flex-1 min-w-0 p-0 m-0 [-moz-appearance:_textfield] [&::-webkit-outer-spin-button]:m-0 [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:m-0 [&::-webkit-inner-spin-button]:appearance-none placeholder-gray-600"
							placeholder="0"
						/>
					</div>
				</div>
							...
							{#if category && categories.find((c) => c.category === category)?.icon_name && iconMap[categories.find((c) => c.category === category)?.icon_name]}
								<picture>
									{#if iconMap[categories.find((c) => c.category === category)?.icon_name].avif}
										<source
											srcset={iconMap[categories.find((c) => c.category === category)?.icon_name]
												.avif}
											type="image/avif"
										/>
									{/if}
									<img
										src={iconMap[categories.find((c) => c.category === category)?.icon_name].webp}
										alt=""
								...
								>
									{#if cat.icon_name && iconMap[cat.icon_name]}
										<picture>
											...
											{/if}
											<img
												src={iconMap[cat.icon_name].webp}
												alt=""
